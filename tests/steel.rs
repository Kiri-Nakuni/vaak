...
t!(写像へ重ねる, "var m : i64 i64 map := ( 1 => 5 ); m[1] += 3; m[1]");
t!(写像へ掛ける, "var m : i64 i64 map := ( 1 => 5 ); m[1] *= 3; m[1]");
t!(文字列の鍵へ重ねる, "var m : str i64 map := ( \"a\" => 5 ); m[\"a\"] -= 2; m[\"a\"]");
t!(hashへ重ねる, "var h : i64 i64 hash := ( 1 => 5 ); h[1] += 3; h[1]");
t!(hashへ掛ける, "var h : i64 i64 hash := ( 1 => 5 ); h[1] *= 3; h[1]");
t!(hashの文字列の鍵へ重ねる, "var h : str i64 hash := ( \"a\" => 5 ); h[\"a\"] -= 2; h[\"a\"]");
t!(重ねても個数は変わらない,
...
t!(並べた方, "fn f () { { { break break break 5 } ; 1 } } -> i64; f() ?? 99");
t!(動く回数でも繋がる,
