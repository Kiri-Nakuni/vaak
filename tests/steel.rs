...
fn agree(name: &str, src: &str) {
    ...
    let Some(got) = steel_run(name, src) else {
        return;
    };
    ...
    assert_eq!(
        got8, expect,
        "{name}: 参照 {want} → {expect}、STEEL {got} → {got8}\n{src}"
    );
}
...
t!(
    ループの値は回数,
    "var n := 0; loop { n += 1; if (n == 5) break; fi; }"
);
...
t!(
    関数,
    "fn add (a : i64, b : i64) { a + b } -> i64; add(20, 22)"
);
t!(
    再帰,
    "fn f (n : i64) { if (n < 2) n else f(n - 1) + f(n - 2) fi } -> i64; f(10)"
);
t!(
    関数から脱出,
    "fn f (n : i64) { nfor (i, 0, n) { if (i == 3) break break i; fi; }; 0 } -> i64; f(10)"
);
t!(型を指定した束縛, "var x : i32 := 300; x");
t!(狭い型で折り返す, "var x : u8 := 200; x + 100");
t!(狭い型への代入, "var x : u8 := 0; x := 300; x");
t!(
    狭い型の欄への代入,
    "struct P { var x : u8 := 0; }; var p := new P (); p.x := 300; p.x"
);
t!(狭い型の値引数, "fn f (x : u8) { x } -> u8; f(300)");
t!(狭い型の返り値, "fn f () { 300 } -> u8; f()");
t!(
    continueで飛ばす,
    "var s := 0; nfor (i, 0, 10) { if (i == 3) continue; fi; s += 1; }; s"
);
...
t!(
    浮動小数の加算,
    "var x := 1.5; var y := 2.25; if (x + y == 3.75) 1 else 0 fi"
);
t!(
    浮動小数の除算,
    "var x := 7.0; var y := 2.0; if (x / y == 3.5) 1 else 0 fi"
);
t!(
    零で割ると畳まれる浮動小数,
    "var x := 1.0; var y := 0.0; if ((x / y) ?? 42.0 == 42.0) 1 else 0 fi"
);
t!(
    非有限は畳まれる,
    "var x := 1.0; var y := 0.0; if ((x / y) ?? 0.0 == 0.0) 7 else 8 fi"
);
t!(浮動小数の比較, "var x := 1.5; if (x < 2.0) 3 else 4 fi");
t!(
    浮動小数の符号反転,
    "var x := 1.5; if (0.0 - x < 0.0) 5 else 6 fi"
);
t!(
    浮動小数はブロックを出ても小数部を保つ,
...
t!(
    浮動小数はcoalesceの合流でも小数部を保つ,
    "if ((1.5 ?? 2.5) == 1.5) 1 else 0 fi"
);
t!(
    浮動小数はswitchの合流でも小数部を保つ,
...
t!(
    f32を指定する,
    "var x : f32 := 1.5; if (x == 1.5 -> f32) 9 else 8 fi"
);
t!(
    f32演算で溢れると畳まれる,
    "var x : f32 := 3.0e38;
...
t!(
    パイプ,
    "fn double (n : i64) { n * 2 } -> i64; 21 |> double()"
);
t!(
    パイプで引数を足す,
    "fn add (a : i64, b : i64) { a + b } -> i64; 20 |> add(22)"
);
t!(
    パイプを繋ぐ,
    "fn inc (n : i64) { n + 1 } -> i64; fn dbl (n : i64) { n * 2 } -> i64; 20 |> inc() |> dbl()"
);
...
t!(
    flowで名前を付ける,
    "flow $out = break break; loop { { $out 8; }; }"
);
t!(
    returnの定番,
...
t!(
    newの既定は零,
    "var a : i64 array := new i64 array(3, 0); a[1] + 1"
);
t!(要素に書く, "var a := [1,2,3]; a[1] := 9; a[1]");
t!(
    合計する,
    "var s := 0; let xs := [1,2,3,4]; nfor (i,0,xs.len()) { s += xs[i]; }; s"
);
t!(
    深く複製する,
    "var a := [1,2,3]; var b := a; b[0] := 9; a[0]"
);
t!(
    複製した方は変わる,
    "var a := [1,2,3]; var b := a; b[0] := 9; b[0]"
);
t!(
    aliasで受ければ写さない,
...
t!(
    配列を返す,
    "fn mk () { [ 3, 1, 4 ] } -> i64 array; let a := mk(); a[0] + a[2]"
);
t!(文字列の長さ, "let s := \"abcd\"; s.len()");
t!(文字列の要素, "let s := \"abcd\"; s[1] -> i64");
t!(
    文字列escapeはbyteとunicodeの違いを保つ,
    r#"let bytes := "\x80\r";
      let unicode := "\u{80}";
...
t!(
    文字列も深く複製する,
    "var s := \"ab\"; var t := s; t[0] := 122; s[0] -> i64"
);
t!(
    u8の配列,
    "var a : u8 array := new u8 array(3, 200); (a[0] -> i64) + 1"
);
t!(
    真偽の配列,
    "var a : bool array := new bool array(3, true); if (a[1]) 7 else 8 fi"
);
t!(
    浮動小数の配列,
    "var a : f64 array := new f64 array(2, 1.5); if (a[0] + a[1] == 3.0) 5 else 6 fi"
);
t!(
    二分探索,
    "flow $return = $repeat(break, getdepth());
...
fn f80はブロックを出ても小数部を保つ() {
    let Some(c) = steel_only("ブロック", "if ({ 1.5 -> f80 } == 1.5 -> f80) 7 else 8 fi")
    else {
        return;
    };
    assert_eq!(c, 7);
...
t!(
    文字列の配列,
    "let xs := [ \"ab\", \"cde\" ]; xs[1].len() * 10 + xs[0].len()"
);
...
t!(
    注釈は集合体にも付く,
    "var a := new i64 array(2, 4); (a -> i64 array)[1]"
);
...
t!(
    別名へ書くと元も変わる,
    "var x := 7; var y : i64 alias &= x; y := 9; x"
);
t!(
    元へ書くと別名も変わる,
    "var x := 7; var y : i64 alias &= x; x := 4; y"
);
t!(
    別名を指し直す,
...
t!(
    別名の別名,
    "var x := 5; var y : i64 alias &= x; var z : i64 alias &= y; z := 3; x"
);
t!(
    別名へ複合代入,
    "var x := 10; var y : i64 alias &= x; y += 5; x"
);
t!(
    集合体の別名,
    "var a := new i64 array(2, 3); var b : i64 array alias &= a; b[0] := 9; a[0] + a[1]"
);
t!(
    別名は枠を新しく作らない,
...
t!(
    押して伸ばす,
    "var a := new i64 array(0, 0); a.push(3); a.push(4); a.push(5); a.len()"
);
t!(
    押した値が読める,
    "var a := new i64 array(0, 0); a.push(7); a.push(9); a[0] * 10 + a[1]"
);
t!(何度も押す,
...
t!(
    引いた値が出る,
    "var a := new i64 array(0, 0); a.push(6); a.push(8); a.pop()"
);
t!(
    空から引くと虚無,
    "var a := new i64 array(0, 0); a.pop() ?? 42"
);
t!(空にする, "var a := new i64 array(9, 1); a.clear(); a.len()");
t!(
    空にしてから押す,
    "var a := new i64 array(9, 1); a.clear(); a.push(3); a.len() * 10 + a[0]"
);
t!(
    押しても他は変わらない,
...
t!(
    先頭へ挿す,
    "var a := new i64 array(2, 5); a.insert(0, 9); a[0] * 100 + a[1] * 10 + a[2]"
);
t!(
    途中へ挿す,
    "var a := new i64 array(0, 0); a.push(1); a.push(3); a.insert(1, 2); a[0]*100+a[1]*10+a[2]"
);
t!(
    末尾へ挿す,
    "var a := new i64 array(0, 0); a.push(1); a.insert(1, 7); a[1] * 10 + a.len()"
);
t!(
    枠の外へ挿すと何も起きない,
    "var a := new i64 array(2, 4); a.insert(5, 9); a.len() * 10 + a[0]"
);
t!(
    負の添字へ挿すと何も起きない,
    "var a := new i64 array(2, 4); a.insert(0 - 1, 9); a.len()"
);
t!(
    抜くと詰まる,
    "var a := new i64 array(0, 0); a.push(1); a.push(2); a.push(3); a.remove(1); a[0]*10+a[1]"
);
t!(
    抜いた値が出る,
    "var a := new i64 array(0, 0); a.push(7); a.push(8); a.remove(0)"
);
t!(
    抜くと減る,
    "var a := new i64 array(4, 1); a.remove(2); a.len()"
);
t!(
    枠の外を抜くと虚無,
    "var a := new i64 array(2, 1); a.remove(9) ?? 42"
);
t!(
    枠の外を抜いても減らない,
    "var a := new i64 array(2, 1); a.remove(9) ?? 0; a.len()"
);
t!(
    末尾を抜く,
    "var a := new i64 array(0, 0); a.push(4); a.push(6); a.remove(1) * 10 + a.len()"
);
t!(
    挿してから抜く,
    "var a := new i64 array(0, 0); a.push(1); a.insert(0, 9); a.remove(1) + a[0]"
);
t!(
    浮動小数を挿す,
...
t!(
    写像リテラル,
    "var m : i64 i64 map := ( 1 => 10, 2 => 20 ); m.len()"
);
t!(
    鍵で引く,
    "var m : i64 i64 map := ( 1 => 10, 2 => 20 ); m[2]"
);
t!(
    無い鍵は虚無,
    "var m : i64 i64 map := ( 1 => 10 ); m[9] ?? 42"
);
t!(
    鍵を足す,
    "var m := new i64 i64 map ( ); m[5] := 7; m[5] * 10 + m.len()"
);
t!(
    同じ鍵は上書き,
    "var m := new i64 i64 map ( ); m[5] := 7; m[5] := 9; m[5] * 10 + m.len()"
);
...
t!(
    鍵があるか,
    "var m : i64 i64 map := ( 1 => 10, 5 => 50 ); if (m.has(5)) 1 else 0 fi"
);
t!(
    無い鍵は無い,
    "var m : i64 i64 map := ( 1 => 10 ); if (m.has(9)) 1 else 0 fi"
);
t!(
    写像から抜く,
    "var m : i64 i64 map := ( 1 => 10, 2 => 20 ); m.remove(1) + m.len()"
);
t!(
    無い鍵を抜くと虚無,
    "var m : i64 i64 map := ( 1 => 10 ); (m.remove(9) ?? 5) + m.len()"
);
t!(
    抜いた後は引けない,
    "var m : i64 i64 map := ( 1 => 10, 2 => 20 ); m.remove(2) ?? 0; m[2] ?? 7"
);
t!(
    写像も深く複製する,
...
t!(
    写像を空にする,
    "var m : i64 i64 map := ( 1 => 1, 2 => 2 ); m.clear(); m.len()"
);
t!(
    文字列の鍵,
...
t!(
    文字列の鍵は短い順,
    "var m : str i64 map := ( \"bb\" => 2, \"a\" => 1 ); m.keys()[0].len()"
);
t!(
    写像の値が集合体,
...
t!(
    hashリテラル,
    "var h : i64 i64 hash := ( 1 => 10, 2 => 20 ); h.len()"
);
t!(
    hashを引く,
    "var h : i64 i64 hash := ( 1 => 10, 2 => 20 ); h[2]"
);
t!(
    hashの無い鍵は虚無,
    "var h : i64 i64 hash := ( 1 => 10 ); h[9] ?? 42"
);
t!(
    hashへ足す,
    "var h := new i64 i64 hash ( ); h[5] := 7; h[5] * 10 + h.len()"
);
t!(
    hashは上書きする,
    "var h := new i64 i64 hash ( ); h[5] := 7; h[5] := 9; h[5] * 10 + h.len()"
);
t!(
    hashは入れた順,
...
t!(
    hashで在るか,
    "var h : i64 i64 hash := ( 1 => 1, 5 => 5 ); if (h.has(5)) 1 else 0 fi"
);
t!(
    hashで無いものは無い,
    "var h : i64 i64 hash := ( 1 => 1 ); if (h.has(9)) 1 else 0 fi"
);
t!(
    hashから抜く,
    "var h : i64 i64 hash := ( 1 => 10, 2 => 20 ); h.remove(1) + h.len()"
);
t!(
    hashの無い鍵を抜くと虚無,
    "var h : i64 i64 hash := ( 1 => 10 ); (h.remove(9) ?? 5) + h.len()"
);
t!(
    hashは抜いた後引けない,
    "var h : i64 i64 hash := ( 1 => 1, 2 => 2 ); h.remove(2) ?? 0; h[2] ?? 7"
);
t!(
    hashを空にする,
    "var h : i64 i64 hash := ( 1 => 1, 2 => 2 ); h.clear(); h.len()"
);
t!(
    hashも深く複製する,
...
t!(
    浮動小数の鍵を引く,
    "var m : f64 i64 map := ( 1.5 => 7, 2.5 => 9 ); m[2.5]"
);
t!(
    hashで浮動小数の鍵を引く,
    "var h : f64 i64 hash := ( 1.5 => 7, 2.5 => 9 ); h[2.5]"
);
t!(
    浮動小数の鍵で無いもの,
    "var m : f64 i64 map := ( 1.5 => 7 ); m[9.5] ?? 42"
);
t!(
    hashで浮動小数の無い鍵,
    "var h : f64 i64 hash := ( 1.5 => 7 ); h[9.5] ?? 42"
);
t!(
    f32の鍵,
    "var m : f32 i64 map := ( 1.5 => 7, 0.0 - 2.5 => 9 ); var k := m.keys(); m[k[0]]"
);
t!(
    hashでf32の鍵,
    "var h : f32 i64 hash := ( 1.5 => 7, 0.0 - 2.5 => 9 ); h[1.5]"
);
t!(
    浮動小数の鍵を抜く,
    "var m : f64 i64 map := ( 1.5 => 7, 2.5 => 9 ); m.remove(1.5) + m.len()"
);
t!(
    hashで浮動小数の鍵を抜く,
    "var h : f64 i64 hash := ( 1.5 => 7, 2.5 => 9 ); h.remove(1.5) + h.len()"
);
...
t!(
    ifの枝へ届く,
    "var x : u8 := if (1 == 1) 100 * 3 / 2 else 0 fi; x"
);
t!(注釈から届く, "(100 * 3 / 2) -> u8");
...
t!(
    欄へ届く,
    "struct P { var x : u8 := 0; }; var p := new P ( x := 100 * 3 / 2 ); p.x"
);
t!(
    欄の既定へも届く,
    "struct P { var x : u8 := 100 * 3 / 2; }; var p := new P (); p.x"
);
t!(
    配列の要素へ届く,
    "var a : u8 array := [ 100 * 3 / 2 ]; a[0]"
);
t!(
    名前で書いても同じ,
...
t!(
    立っているビットを数える,
    "var x : u8 := 0b1011; x.count_ones()"
);
t!(
    前の零を数える,
    "var x : u8 := 0b00010000; x.leading_zeros()"
);
t!(零の前の零, "var x : u8 := 0; x.leading_zeros()");
t!(
    後ろの零を数える,
    "var x : u8 := 0b00010000; x.trailing_zeros()"
);
t!(零の後ろの零, "var x : u8 := 0; x.trailing_zeros()");
t!(
    ビットを逆にする,
    "var x : u8 := 0b11010000; x.reverse_bits()"
);
t!(バイトを入れ替える, "var x : u16 := 0x1234; x.swap_bytes()");
t!(
    一バイトは入れ替えても同じ,
    "var x : u8 := 0x12; x.swap_bytes()"
);
t!(左へ回す, "var x : u8 := 1; x.rotate_left(1)");
t!(
    幅だけ回すと戻る,
    "var x : u8 := 0b10110001; x.rotate_left(8)"
);
t!(幅を越えて回す, "var x : u8 := 1; x.rotate_left(9)");
t!(右へ回す, "var x : u8 := 1; x.rotate_right(1)");
t!(
    端をまたいで回す,
    "var x : u8 := 0b10000000; x.rotate_left(1)"
);
t!(足して止まる, "var x : u8 := 250; x.saturating_add(10)");
t!(引いて止まる, "var x : u8 := 5; x.saturating_sub(10)");
t!(
    掛けて止まる,
    "var x : i32 := 100000; x.saturating_mul(100000)"
);
t!(
    負へ掛けて止まる,
    "var x : i32 := 0 - 100000; x.saturating_mul(100000)"
);
t!(止まらない掛け算, "var x : u8 := 20; x.saturating_mul(20)");
t!(
    平方根,
    "var x : f64 := 4.0; if (x.sqrt() == 2.0) 1 else 0 fi"
);
t!(
    負の平方根は虚無,
    "var x : f64 := 0.0 - 1.0; if ((x.sqrt() ?? 7.0) == 7.0) 1 else 0 fi"
);
t!(
    浮動小数の絶対値,
    "var x : f64 := 0.0 - 2.5; if (x.abs() == 2.5) 1 else 0 fi"
);
t!(
    床と天井,
...
t!(
    切り捨て,
    "var x : f64 := 0.0 - 2.5; if (x.trunc() == 0.0 - 2.0) 1 else 0 fi"
);
t!(
    四捨五入,
    "var x : f64 := 2.5; if (x.round() == 3.0) 1 else 0 fi"
);
t!(
    符号を写す,
    "var x : f64 := 3.0; if (x.copysign(0.0 - 1.0) == 0.0 - 3.0) 1 else 0 fi"
);
t!(
    掛けて足す,
    "var x : f64 := 2.0; if (x.mul_add(3.0, 1.0) == 7.0) 1 else 0 fi"
);
t!(
    冪,
    "var x : f64 := 2.0; if (x.pow(10.0) == 1024.0) 1 else 0 fi"
);
t!(
    零の対数は虚無,
    "var x : f64 := 0.0; if ((x.ln() ?? 5.0) == 5.0) 1 else 0 fi"
);
t!(
    二の対数,
    "var x : f64 := 8.0; if (x.log2() == 3.0) 1 else 0 fi"
);
t!(
    f32の平方根,
    "var x : f32 := 4.0; if (x.sqrt() == 2.0) 1 else 0 fi"
);
...
t!(
    動く段数で二段,
    "var n := 2; {{ { $repeat(break, n) 7 } ; 3 }}"
);
t!(
    動く段数で一段だけ抜ける,
    "var n := 1; {{ { $repeat(break, n) 7 } ; 3 }}"
);
t!(
    動く段数で三段,
...
t!(
    条件つきで遅延する,
    "nfor (i, 0, 10) { if (i < 3) continue break i fi; }"
);
...
t!(
    越えて一段抜ける,
    "fn f () { break outward break 1; } -> i64; {{ f() }}"
);
t!(
    越えた先で二段抜ける,
...
t!(
    写像へ重ねる,
    "var m : i64 i64 map := ( 1 => 5 ); m[1] += 3; m[1]"
);
t!(
    写像へ掛ける,
    "var m : i64 i64 map := ( 1 => 5 ); m[1] *= 3; m[1]"
...
t!(
    文字列の鍵へ重ねる,
    "var m : str i64 map := ( \"a\" => 5 ); m[\"a\"] -= 2; m[\"a\"]"
);
t!(
    hashへ重ねる,
    "var h : i64 i64 hash := ( 1 => 5 ); h[1] += 3; h[1]"
);
t!(
    hashへ掛ける,
    "var h : i64 i64 hash := ( 1 => 5 ); h[1] *= 3; h[1]"
...
t!(
    hashの文字列の鍵へ重ねる,
    "var h : str i64 hash := ( \"a\" => 5 ); h[\"a\"] -= 2; h[\"a\"]"
);
t!(
    重ねても個数は変わらない,
    "var m : i64 i64 map := ( 1 => 5, 2 => 6 ); m[1] += 3; m.len() * 100 + m[1]"
);
...
t!(
    零段でも被演算子の脱出は残る,
    "fn f () { { $repeat(break, 0) break 5 } } -> i64; f() ?? 99"
);
t!(
    一段は被演算子と繋がる,
    "fn f () { { $repeat(break, 1) break 5 } } -> i64; f() ?? 99"
);
t!(
    動く零段でも被演算子は残る,
    "fn f () { var n := 0; { $repeat(break, n) break 7 } } -> i64; f() ?? 99"
);
t!(
    動く一段も被演算子と繋がる,
    "fn f () { var n := 1; { $repeat(break, n) break 7 } } -> i64; f() ?? 99"
);
t!(
    畳んでも並べても同じ,
    "fn f () { { { $repeat(break, 2) break 5 } ; 1 } } -> i64; f() ?? 99"
);
t!(
    並べた方,
    "fn f () { { { break break break 5 } ; 1 } } -> i64; f() ?? 99"
);
t!(
    動く回数でも繋がる,
