...
/// 写像・`hash` への複合代入。**無い鍵は誤りである**（挿さない）。
#[test]
fn 連想への複合代入() {
    同じ("var m : i64 i64 map := ( 1 => 5 ); m[1] += 3; m[1]");
    同じ("var m : i64 i64 map := ( 1 => 5 ); m[1] *= 3; m[1]");
    同じ("var m : str i64 map := ( \"a\" => 5 ); m[\"a\"] -= 2; m[\"a\"]");
    同じ("var h : i64 i64 hash := ( 1 => 5 ); h[1] += 3; h[1]");
    同じ("var h : i64 i64 hash := ( 1 => 5 ); h[1] *= 3; h[1]");
...
fn repeat_の被演算子が作用素式() {
    ...
    同じ("fn f () { var n := 0; { $repeat(break, n) break 7 } } -> i64; f() ?? 99");
