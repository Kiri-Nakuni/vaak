...
### 2026-08-26 STEEL修正後の再観測

上の失敗記録は削除しない。原因は集合体返値の`ok = false`側でもnull仮値を深い複製していたことで、
値がある側だけを複製する制御流へ直した。同じfixtureは終了値42となったため、試験名を
`steelの負長constructor回収は参照実装とvmに一致する`へ変え、`#[ignore]`を外した。

同時に、STEELが`&&` / `||`をLLVMの eager な`and` / `or`へ落としていた不一致を修正した。
右辺を実行するかを分岐することで、graph 3件、heap、ASCII I/Oのnative fixtureが通った。string fixtureは
実装結果`A--あ`が非重複全置換と空欄を保つsplit契約に合っており、`A-Aあ`という試験期待だけを直した。
全release gateは844 passed、0 failed、0 ignoredである。

旧eager案は正しくないが、捨てる理由を性能値ごと残す。xorshiftを5,000万周し、安全な右辺
`(x mod 97) < 31`を評価するclang 18.1.3 `-O2` nativeをhyperfineで測った（5回予熱、25回）。
左辺がランダムに約半分真のとき、旧eagerは159.1 ± 3.9 ms、短絡版は253.0 ± 14.4 msで、短絡版が
約1.59倍遅かった。分岐予測失敗の費用である。一方、左辺が約0.1%だけ真なら15回平均は旧
177.8 ± 10.3 ms、短絡95.7 ± 3.2 msで、短絡版が約1.86倍速かった。約99.9%真の25回は旧
226.5 ± 28.5 ms、短絡217.2 ± 35.7 msで測定noise内だった。旧案は右辺の範囲外読出しや
paradox、書換えまで実行して誤答・SIGSEGVになるため、速い50%合成値を理由に戻さない。将来、右辺が
副作用なし・paradoxなしと証明できる場合だけ、別のtotal-expression最適化として再検討する。
