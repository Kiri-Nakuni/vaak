...
ほぼ線形になった。この時点では参照実装にまだ超線形な費用が残ったが、
少なくとも専用 stack の追加で解ける問題ではなかった。既存 array の一度の修正が、
token buffer、AST arena、operator stack、評価 stack のすべてへ効いた。

### method dispatch の型確認も借用した後

参照実装は組み込みメソッドへ到達する前に、利用者定義メソッドを探すためだけに
名前レシーバ全体を `need_value` で複製していた。cell から `type_of()` だけを読むようにし、
もう一度 11 標本で測った。

| 左深さ | tokens | nodes | visits | 参照実装 | VM | VM / 参照 |
|---:|---:|---:|---:|---:|---:|---:|
| 8 | 33 | 17 | 25 | 0.998 ms | 0.392 ms | 0.393 |
| 32 | 129 | 65 | 97 | 2.906 ms | 1.466 ms | 0.505 |
| 64 | 257 | 129 | 193 | 5.599 ms | 2.663 ms | 0.476 |
| 128 | 513 | 257 | 385 | 12.107 ms | 5.822 ms | 0.481 |

32 から 128 への仕事量約 4 倍に対し、参照実装は約 4.2 倍、VM は約 4.0 倍である。
最初の深さ 128 と比べると参照実装は 207.718 ms から 12.107 ms（約 17.2 倍）、
VM は 73.143 ms から 5.822 ms（約 12.6 倍）になった。二実行器とも主要経路は線形で、
array work stack が専用 stack より計算量で不利だという仮説はこの測定では支持されない。
