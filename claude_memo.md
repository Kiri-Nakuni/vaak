...
## 積んでいるもの（2026-08-22 時点）

### STEEL
`outward`（C-34 が非推奨と書いた難物）・動く段数の `$repeat`・
被演算子つきの `continue`・利用者定義のメンバ関数・ホスト界面。

### 数値型の組み込み関数【依頼者から追加】
**LLVM の命令にあるものを片端から。** 各種数値型に対して。

見込みで挙げると:

| | |
|---|---|
| 整数 | `abs` `min` `max` `popcount` `ctlz` `cttz` `bswap` `bitreverse` |
| | 回転（`fshl` / `fshr`）・飽和演算（`sadd.sat` など） |
| | 掛け算の上位（`smul` の高位ビット）・除算と剰余の同時 |
| 浮動小数 | `sqrt` `fma` `abs` `min` `max` `floor` `ceil` `trunc` `round` |
| | `copysign` `exp` `log` `pow` `sin` `cos` |

**三実装ぶん要る。** 木を辿る実装が参照（S-5）なので、Rust の同名の関数で書けるものが多い。
`f80` は STEEL だけなので差分試験の相手がいない（S-20）——そこは別扱い。

C-7（「1文字」と言わない、名前が何を数えているか言う）に倣って、
**名前は何をするか言うもの**にすること。

### Codex からの提案（未決定）
`arena` を一級機能にする案が `for_CLAUDE.md` に来ている。**依頼者の決定待ち。**
